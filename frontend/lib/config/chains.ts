...
export function getDefaultChain(): ChainConfig {
  ...
  // Check for NEXT_PUBLIC_DEFAULT_CHAIN env variable
  const defaultChainName = process.env.NEXT_PUBLIC_DEFAULT_CHAIN?.toLowerCase()
  if (defaultChainName) {
    const defaultChain = chains.find(
      (c) => c.name.toLowerCase().replace(/\s+/g, '-') === defaultChainName
    )
    if (defaultChain) {
      return defaultChain
    }
    console.warn(`[Chain Config] Default chain "${defaultChainName}" not found in supported chains, using first available`)
  }
